...
export default class History2 {
   ...
   *     page is loaded.
   */
  constructor({onBeforeChange, onAfterChange} = {}) {
    this.onBeforeChange = onBeforeChange || noop;
    this.onAfterChange = onAfterChange || noop;
  ...
  async update({url, title, isPopState}) {
    ...
    await this.onBeforeChange(nextState);
    ...
    await this.onAfterChange(nextState);
  }
}
